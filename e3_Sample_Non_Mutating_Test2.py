...
def test_get_department_leader():
    assert L58.get_department_leader() == L58
    for item in [L81, e48, e38]:
        assert item.get_department_leader() == L81
    for item in [e3, e23, e88, e41, e42, e43, e51, e61, e7, e30]:
        assert item.get_department_leader() is None
    for item in [e4, e5, e6, L80]:
        assert item.get_department_leader() == L80
    assert L77.get_department_leader() == L77
    assert L78.get_department_leader() == L78
    assert L82.get_department_leader() == L82
    assert L83.get_department_leader() == L83
    assert L84.get_department_leader() == L84
    assert L85.get_department_leader() == L85
    assert L86.get_department_leader() == L86
    assert L39.get_department_leader() == L39
    assert single_l.get_department_leader() == single_l
    assert single_e.get_department_leader() is None
if __name__ == '__main__':
    import pytest

    pytest.main(['e3_Sample_Non_Mutating_Test2.py'])
